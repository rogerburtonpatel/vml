...
\subsection{Expressions}
    ...
    \newcommand\EvalE{\vmrun[Expr=\alpha] \Downarrow \vartheta}
    \newcommand\EvalGe{\vmrun \Downarrow \result}
...
\inferrule*[Left=\textsc{ (Eval-ArrowExpr) }]
    {\vmrun[context=\emptyseq] \Downarrow \vartheta}
    {\vmrun[context=\emptyseq, Expr=\arrowe] 
    \Downarrow \vartheta}
\]

\[
\inferrule*[Left=\textsc{ (Eval-Exists) }]
    {\vmrun[env=\rho\{x \mapsto \bot \}] \Downarrow \result}
    {\vmrun[Expr=\exists x.\; \galpha] 
    \Downarrow \result}
\]
...
\inferrule*[Left=\textsc{ (G-Move-To-Ctx) }]
    ...
    {\vmrun[Expr=eq; \; \galpha] \Downarrow \result}
\]
...
\inferrule*[Left=\textsc{ (G-EqExps) }]
    ...
    \\\\
    \vmrun[Rhoext=\bracketed{x \mapsto \bot,\; y \mapsto \bot},
          context=\eqTT{x = \ealphaone \cdot y = \ealphatwo \cdot x = y}]
    \Downarrow \result 
    }}
    {\vmrun[context=\TeqT{\ealphaone = \ealphatwo}] 
    \Downarrow \result}
\]
...
\inferrule*[Left=\textsc{ (G-EqNameExp) }]
    ...
    \veval{\ealpha}{\valpha}
    \and 
    \vmrun[Rhoext=\bracketed{x \mapsto \valpha},
           context=\context \cdot \context']
    \Downarrow \result'
    }}
    {\vmrun[context=\TeqT{x = \ealpha}] 
    \Downarrow \result'}
\]
...
\inferrule*[Left=\textsc{ (G-Vcon-Multi-Fail) }]
    {K \neq K'}
    {\vmrun[context=\TeqT{K(\ealphaone, \dots \ealphan) = K'(\ealphaone', \dots \ealphan')}] 
    \Downarrow \reject}
\]
...
\inferrule*[Left=\textsc{ (G-Vcon-Multi-Arity-Fail) }]
    {n \neq m}
    {\vmrun[context=\TeqT{K(\ealphaone, \dots \ealphan) = K(\ealphaone', \dots e_{\alpha_{m}}')}] 
    \Downarrow \reject}
\]
...
\inferrule*[Left=\textsc{ (G-Vcon-Multi-Succ) }]
    {
    \vmrun[context=\eqTT{\lbrack \ealphai=\ealphai' \; \vert \; 1 \leq i \leq n \rbrack}] 
    \Downarrow \result}
    {\vmrun[context=\TeqT{K(\ealphaone, \dots \ealphan) = K(\ealphaone', \dots \ealphan')}] 
    \Downarrow \result}
\]
...
\inferrule*[Left=\textsc{ (EqNames-Bots-Succ) }]
    ...
    \rho(x) = \bot, \; \rho(y) = \bot
    \\\\
    \rm{Either} \; x \; \rm{or} \; y \; \rm{appears in} \; \context, \; \context'
    \\\\
    \vmrun[context=\TeqT{x = y}] 
    \Downarrow \result 
    }}
