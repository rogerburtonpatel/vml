...
\begin{center}
    ...
    \B{prog} : \textsf{Program} ::=
    $\bracketed{\B{def}}$
    ;;
    \B{def} : \textsf{Definitions} ::=
    | $\tt{(val} \; \B{name} \; \B{exp}\tt{)}$
    ;;
    ...
    | \B{exp} \B{exp}
    ;;
    \B{case} : \textsf{Case Expressions} ::= 
    \tt{(case} \B{exp} \{ \B{case-branch} \} \tt{)}
    % <case> ::= \tt{(case} <expr> \{ <case-branch> \} \tt{)}
    ;;
    \B{case-branch} ::= \tt{(}\B{pattern} \B{exp} \tt{)}
    ;;
    \B{pattern} ::= \B{name}
    | \B{K}
    | \tt{(}\B{K} \{\B{pattern}\}\tt{)}
    | \tt{(}\B{pattern} \tt{when} \B{exp} \tt{)} 
    ;;
    \B{K} : \textsf{Value Constructors} ::=
    \cons 
    | \tt{[]} 
    | token beginning with a capital letter or a colon
    | \B{i}
    ;;
    \B{i} ::=  token composed only of digits, 
                             possibly prefixed with a \tt{+} or \tt{-}.
...
A \B{name} is any token that is not an \B{integer-literal}, 
does not contain whitespace, a bracket, or parenthesis, 
and is not a \B{K} or a reserved word.
