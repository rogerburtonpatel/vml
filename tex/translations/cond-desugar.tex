...
A desugaring of Scheme \texttt{cond} to \textit{V}: 

\begin{align*}
\texttt{(cond)} \;\; & \triangleq \;\; \texttt{wrong} \\
\texttt{(cond [} e_g, \; e_a \texttt{] ...)} \;\; 
& \triangleq \;\; \texttt{if} \;\; e_g  \;\; \texttt{then} \;\; e_a \;\; 
  \texttt{else (cond ...)} \\
\end{align*}

where $e_g$ is a \textit{guarded-exp} and $e_a$ is an \textit{exp} on the 
right-hand side. 
