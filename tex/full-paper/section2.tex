...
fun area sh =
  case sh 
    ...
    calls this a \it{head expression}. I follow Ramsey's~\citep{bpc} and
    Maragnet's~\citep{maranget} example in calling the things \it{case} and
...
*)
    ...
    modify an example from Erwig \& Peyton Jones~\citep{guardproposal}, the
...
$\exists$ game_of_token. game_of_token = $\lambda$ token. 
  $\exists$ f. one {
    ...
    Maranget~\citep{maranget} showed that pattern matching can be compiled to a
