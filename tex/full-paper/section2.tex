...
$\exists$ game_of_token. game_of_token = $\lambda$ token. 
  $\exists$ f. one {
    ...
    functional logic languages, relies on unifying its logical variables[cite]
    and search[cite] at runtime to meet a set of program constraints [cite].
    Combining unifying logical variables with search at runtime classically
    requires backtracking [cite], which can lead to exponential runtime cost
    [cite]. 

    TODO NEEDS REWRITE: Combine these 2 parts $\uparrow \downarrow$ and 
    say what you mean: choice + names as values == unification and search. 
    Those 2 together, particularly search, need backtracking. NEED AN EXAMPLE. 
    Evaluating a function backwards is good enough. 
