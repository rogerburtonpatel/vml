...
\section{Languages are Interesting}
    ...
    In most functional languages, the \tt{case} can be removed entirely in a
    form of syntactic sugar known as a \it{clausal definition}:
    ...
    \caption{\tt{clunky} with pattern gaurds} 
...
\begin{outline}[enumerate]
    ...
    \2 \bf{-> Because they reduce to standard things by (direct) application of algebraic laws. }
        ...
        \3 This works because no side fx in \PPlus\ and all names are unique $\uparrow$ 
...
\begin{outline}[enumerate]
    ...
    \2 \bf{Likely inductivey hypothesis. 1-4 sentences on proof max. }
\end{outline}
