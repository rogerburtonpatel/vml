...
% TODO Change this after the whole paper is done Verse is significantly more
...
of extensions. Rather than continuing to extend pattern matching~\it{ad hoc}, a
worthwhile goal could be to find an alternative that doesn't need extensions. A
tempting possibility was introduced by the programming language
Verse~\citep{verse}. In Verse, a programmer can deconstruct data using
~\it{equations}. Equations are flexible, and it appears that they can express
everything that pattern matching can, including the popular extensions. 
...
\label{pmandextensions}
    ...
    functional programming. In order to 

    
    
    but it often requires extensions to simplify cases
    ...
    Consider the examples of pattern matching without extensions in figure 
    TODO, with side-by-side examples of how 
...
\showvjudgement{Compile}{\compilebig}
      ...
      The rules are nondeterministic: the structure of the final result~\expr\
      depends on the order in which rules are applied by the compiler.  
